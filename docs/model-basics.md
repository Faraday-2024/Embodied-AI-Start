...
这一页是 VLA、WM、WAM 和 MBRL 之间的共同模型底座。重点不是记住模型名称，而是能回答：输入是什么、张量如何流动、训练时预测什么、部署时如何得到动作或未来。这里的公式是常见实现的抽象；具体论文可能更换 token 化方式、条件注入位置、噪声路径或采样器。
...
X_{out}=X' + \operatorname{FFN}(\operatorname{LN}(X')).
$$
